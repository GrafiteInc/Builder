...
class CrudGeneratorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->config = [
            ...
            '_path_api_controller_'      => vfsStream::url('routes/Http/Controllers/Api'),
            '_path_views_'               => vfsStream::url('resources/views'),
            '_path_tests_'               => vfsStream::url('tests'),
            '_path_request_'             => vfsStream::url('Http/Requests'),
            '_path_routes_'              => vfsStream::url('routes/web.php'),
            '_path_api_routes_'          => vfsStream::url('routes/api.php'),
            'routes_prefix'              => '',
            'routes_suffix'              => '',
    ...
    {
        $this->crud = vfsStream::setup('routes');
        vfsStream::newFile('api.php')->at($this->crud)->setContent("<?php\n\n\n");
        vfsStream::newFile('Http/Controllers/Api/TestTableController.php')->at($this->crud)->setContent("<?php\n\n\n");
    ...
    {
        $this->crud = vfsStream::setup("routes");
        file_put_contents(vfsStream::url('routes/web.php'), 'test');
        $this->generator->createRoutes($this->config, false);
        $contents = $this->crud->getChild('routes/web.php');
